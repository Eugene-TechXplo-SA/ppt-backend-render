...
from fastapi.responses import StreamingResfrom fastapi import FastAPI, File, UploadFile, HTTPException
...
async def generate(excel: UploadFile = File(...), ppt: UploadFile = File(...), images: UploadFile = File(None)):
    with tempfile.TemporaryDirectory() as tmpdir:
        try:
            ...
            raise HTTPException(status_code=500, detail=f"Internal Server Error: {str(e)}")ponse
from fastapi.middleware.cors import CORSMiddleware
import tempfile
import os
import zipfile
...
def is_image_value(val):
    """Check if value looks like an image path—expanded extensions, case-insensitive."""
    if val and isinstance(val, str):
                ...
                for field in matches:
                    ...
                    if matching_col:
                        ...
                        if is_image_value(img_value):
                            ...
                            logger.warning(f"No image value for {field}: {img_value}")
                    else:
                        logger.warning(f"No matching column for {field}")
    except Exception as e:
        logger.error(f"Error in replace_images_on_shape: {str(e)}")
